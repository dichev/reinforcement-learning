...
if __name__ == '__main__':
    ...
    value_net = ValueBaseline(env.observation_space.shape[0])
    optimizer_value = optim.Adam(params=value_net.parameters(), lr=cfg.LEARN_RATE)
    writer = SummaryWriter(f'runs/REINFORCE baseline=value-net {now()}', flush_secs=2)
    ...
    for epoch in range(1, cfg.EPOCHS+1):
        ...
        # Train the policy network (using the baseline)
        ...
        if epoch % 100 == 0:
            print(f"#{epoch:>5}. loss={loss.item():.4f}, loss_value={loss_value.item():.4f} last_reward={episode.total_rewards:.2f}, avg_reward={avg_reward:.2f}")
            writer.add_scalar('t/Loss', loss, epoch)
            writer.add_scalar('t/Loss Value', loss_value, epoch)
