...
def writer_add_params(writer, model, i):
    ...
    writer.add_scalar(f'{model_name}/Gradients Norm', parameters_to_vector(p.grad for p in model.parameters()).norm(), i)
    writer.add_scalar(f'{model_name}/Weights Norm', parameters_to_vector(model.parameters()).norm(), i)
